...
The devtool dogfoods the framework's `cli` package: the tool that manages the
...
- Module root `github.com/krewire/krewire`; binary name `krewire`.
...
- G2 — Dogfood the framework's `cli` package for every command.
...
| ----------- | ----------------------------------------------------------------------- | -------- |
| RND-MT-001  | Provide a `krewire` CLI whose commands are built on the framework `cli` package. | Must    |
...
- D1 — `cmd/krewire` built on `github.com/krewire/framework/tui` (RND-MT-001).
...
- C1 — The devtool is a Go module; `cmd/krewire` is the only entry point.
...
- S1 — Every ecosystem lifecycle task (scaffold, inspect, validate, report) runs through `krewire`.
