...
**One binary, every workload.** `kiw` is the only entry point (binary `kiw`, module `github.com/krewire/kiw`). New projects start as `kernel` (`go.mod`+`krewire.yaml`+`main.go`+`.gitignore`) and are equipped to a kind via `kiw init` (`--static`/`--book`/`--cli`; `--worker`/`--service`/`--infra` are planned).
